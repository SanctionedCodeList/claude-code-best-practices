...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Claude Code Session History API")
    subparsers = parser.add_subparsers(dest="command", required=True)

    # build command
    build_parser = subparsers.add_parser("build", help="Build/update the session index")
    ...
    # meta command
    meta_parser = subparsers.add_parser("meta", help="Get session metadata")
    meta_parser.add_argument("session_id", help="Session UUID")

    # read command
    read_parser = subparsers.add_parser("read", help="Read session messages")
    read_parser.add_argument("session_id", help="Session UUID")
    read_parser.add_argument("--types", nargs="+", help="Filter by types")
    read_parser.add_argument("--tools", nargs="+", help="Filter tool_use by tool names")
    read_parser.add_argument("--first", type=int, help="First N messages")
    read_parser.add_argument("--last", type=int, help="Last N messages")
    ...
    if args.command == "build":
        stats = build_index(force=args.force, verbose=args.verbose)
        print(json.dumps(stats, indent=2))
    elif args.command == "search":
        results = search(args.query, limit=args.limit, project=args.project)
        print(json.dumps(results, indent=2))
    elif args.command == "meta":
        result = meta(args.session_id)
        print(json.dumps(result, indent=2))
    elif args.command == "read":
        messages = read(args.session_id, types=args.types, tools=args.tools,
                       first=args.first, last=args.last,
                       offset=args.offset, limit=args.limit)
        print(json.dumps(messages, indent=2))
    elif args.command == "list":
        results = list_sessions(project=args.project, limit=args.limit)
        print(json.dumps(results, indent=2))
